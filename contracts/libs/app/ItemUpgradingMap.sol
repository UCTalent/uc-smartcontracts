...
contract ItemUpgradingMap is Auth {
  ...
  function setItem(uint8 _gene, uint16 _itemClass, uint16[] calldata _materials, uint _fee, uint8 _successRatio) external onlyMainAdmin {
  ...
  function updateItem(uint16 _itemClass, uint16[] calldata _materials, uint _fee, uint8 _successRatio) external onlyMainAdmin {
    _validateClass(_itemClass);
  ...
  function updateFee(uint8 _gene, uint _fee) external onlyMainAdmin {
    _validateGene(_gene);
  ...
  function updateRatio(uint16 _classId, uint8 _successRatio) external onlyMainAdmin {
    _validateRatio(_successRatio);
